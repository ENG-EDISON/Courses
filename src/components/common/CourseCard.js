...
const validateCourseData = (course) => {
  const defaults = {
    ...
    certificate_available: false,
    enrollment_count: 0,
    is_enrolled: false, // ADDED: Enrollment status
  };
...
const EnrolledBadge = () => <span className="coursecard-badge enrolled">Enrolled</span>; // ADDED
...
function CourseCard({
  ...
  layout = "vertical",
  className = "",
  isAuthenticated = false, // ADDED: Authentication status
  ...props
}) {
  const validatedCourse = useMemo(() => validateCourseData(course), [course]);
  ...
  // ADDED: Determine the target URL based on authentication and enrollment
  const getCourseLink = () => {
    if (isAuthenticated) {
      // If user is authenticated AND enrolled, go to course content
      if (validatedCourse.is_enrolled) {
        return `/course-content/${validatedCourse.id}`;
      }
      // If authenticated but not enrolled, go to course details page
      return `/course/${validatedCourse.id}`;
    }
    // If not authenticated, always go to course details page
    return `/course/${validatedCourse.id}`;
  };
  ...
  return (
    <div className="coursecard-wrapper">
      <article
        className={`coursecard ${layout} ${className} ${
          validatedCourse.is_enrolled ? 'coursecard--enrolled' : ''
        }`}
        onMouseEnter={onMouseEnter}
        onMouseLeave={onMouseLeave}
        tabIndex={0}
        ...
        {...props}
      >
        <Link to={getCourseLink()} className="coursecard-link">
          {/* IMAGE */}
          <div className="coursecard-image-container">
            <CourseThumbnail
              src={validatedCourse.thumbnail}
            ...
            <div className="coursecard-badges">
              {validatedCourse.is_featured && <FeaturedBadge />}
              {validatedCourse.certificate_available && <CertificateBadge />}
              {validatedCourse.is_enrolled && <EnrolledBadge />} {/* ADDED */}
            </div>
          ...
          <div className="coursecard-content">
            ...
            {/* PRICE - Hide price if enrolled */}
            {!validatedCourse.is_enrolled && (
              <div className="coursecard-pricing">
                <div className="coursecard-price-container">
                  {priceInfo.hasDiscount ? (
                    <>
            ...
            {/* ADDED: Show enrolled message instead of price */}
            {validatedCourse.is_enrolled && (
              <div className="coursecard-enrolled-message">
